...
export const isCoordinate = (longitude: number, latitude: number) =>
  Number.isFinite(longitude) &&
  Number.isFinite(latitude) &&
  longitude >= -180 && longitude <= 180 &&
  latitude >= -90 && latitude <= 90;

/** 取得道路事件 popup 使用的經緯度座標。 */
export function getRoadEventPopupCoordinate(
  event: Pick<RoadEventMapPoint, "longitude" | "latitude">,
): [number, number] {
  return [event.longitude, event.latitude];
}

/** 從 TDX WKT 點或面資料取得代表座標。 */
