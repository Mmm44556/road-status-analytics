...
export default function TrafficSummary() {
  ...
  const cards = [
    { label: "事故總數", value: totals.A1 + totals.A2 + totals.A3, icon: CrisisAlertRoundedIcon, ...uiColors.metric.total },
    { label: "傷亡事故", value: totals.A1 + totals.A2, icon: LocalHospitalRoundedIcon, ...uiColors.metric.casualty },
    { label: "事故最多", value: topCity?.city ?? "—", icon: PlaceRoundedIcon, ...uiColors.metric.location },
  ];
  ...
  return (
    <Paper component="section" aria-labelledby="traffic-summary-title" sx={{ p: 2.25 }}>
      ...
      <Box sx={{ display: "grid", gap: 1 }}>
        {cards.map(({ label, value, icon: Icon, main, soft }) => (
          <Box key={label} sx={{ display: "grid", gridTemplateColumns: "38px 1fr auto", alignItems: "center", gap: 1, py: 1.1, borderTop: "1px solid", borderColor: "divider" }}>
            <Box sx={{ width: 36, height: 36, display: "grid", placeItems: "center", borderRadius: 1.5, bgcolor: soft, color: main }}><Icon fontSize="small" /></Box>
